...
namespace QuizApp.Business.Implementation.Services
{
    public class TopicService : ICrudInterface<TopicDto>
    {
        ...
        {
            return _context.Topics.AsQueryable().Where(t => t.SubjectId.Equals(principalId)).Select(TopicMapper.ProjectToDto).AsAsyncEnumerable();
        }
        ...
        {
            var entity = await _context.Topics.FindAsync(model.Id);
